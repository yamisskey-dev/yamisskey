...
### AI
@claude このPRをレビューしてください。Misskey 2025.11.0への追従とチャンネル関連のUI改善PRです。

#### 主な変更点：

**本家追従**
- Misskey 2025.11.0のマージ（チャンネルミュート機能などを含む）

**UI改善**
- ピン留めチャンネル一覧の空状態表示を追加
- 英語翻訳の簡潔化（"Pin to profile" → "Pin"）
- ユーザープロフィールに管理チャンネル一覧タブを追加

**新機能**
- プライバシー設定「プロフィールのチャンネルを非公開にする」を追加
- 本人・管理者・モデレーターは常に閲覧可能

#### レビュー時の重点確認ポイント：

1. **データベースマイグレーション**: hideProfileChannelsカラムの追加が適切か
2. **プライバシー設定の一貫性**: 他のプライバシー設定（hideProfileFiles等）と同じパターンで実装されているか
3. **UI/UX**: チャンネルタブの配置とデザインが他のタブと整合しているか
4. **ローカライゼーション**: 日本語・英語の翻訳が適切か
5. **権限チェック**: 本人・管理者・モデレーターの閲覧権限が正しく実装されているか
...
- ピン留めボタンのラベルを簡潔に変更
  - `"Pin to profile"` → `"Pin"`
  - `"Unpin from profile"` → `"Unpin"`

### プロフィールに管理チャンネル一覧タブを追加

ユーザープロフィールに「管理しているチャンネル一覧」タブを追加しました。

#### 主な機能

**チャンネルタブの追加**
- ユーザープロフィールの「リスト」と「ページ」の間に「チャンネル」タブを配置
- そのユーザーが管理しているチャンネルの一覧を表示
- チャンネルプレビューをグリッドレイアウトで表示（最大幅1200px）

**プライバシー設定の追加**
- 新しい設定項目「プロフィールのチャンネルを非公開にする」を追加
  - デフォルト: 非公開（テスト環境では公開）
  - 設定画面: 設定 > プライバシー
  - 本人・管理者・モデレーターは設定に関わらず常に閲覧可能

**発見性の向上**
- 他のユーザーのプロフィールから「この人が管理しているチャンネル」を発見可能
- コミュニティ形成やチャンネルへの参加を促進

#### 技術詳細

**実装ファイル**
- `packages/frontend/src/pages/user/channels.vue`: チャンネル一覧コンポーネント
- `packages/frontend/src/pages/user/index.vue`: プロフィールタブにチャンネルタブを追加
- `packages/frontend/src/pages/settings/privacy.vue`: プライバシー設定画面
- `packages/backend/src/models/UserProfile.ts`: hideProfileChannelsカラムを追加
- `packages/backend/migration/1763774102600-add-hide-profile-channels.js`: データベースマイグレーション

**データベーススキーマ変更**
- `user_profile`テーブルに`hideProfileChannels`カラムを追加
  - 型: `boolean`
  - デフォルト: `true`（テスト環境では`false`）

**ローカライゼーション**
- 日本語: `hideProfileChannels`, `hideProfileChannelsDescription`
- 英語: `hideProfileChannels`, `hideProfileChannelsDescription`
