...
### バグ修正
- 匿名アクセスにプロフィール本文を見せない場合（`requireSigninToViewContents` 有効、またはインスタンスの UGC 公開範囲制限）でも、rel=me リンクと acct のみの最小 OGP メタデータを SSR で出力するようにし、他サーバーからの rel=me 検証バッジとリンクプレビューが機能しない問題を修正（#327）。出力するのは AP actor として既に連合へ公開されている情報のみで、bio・表示名・アバター・投稿は引き続き匿名には出さない。
