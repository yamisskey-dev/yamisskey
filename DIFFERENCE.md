...
- 設定: リノート表示系の独自機能設定（たたむトリガー・自分のリノートをたたむ）を「タイムラインとノート」から「メンタルヘルス」カテゴリに移動（#334）。
- 設定: リアクション関連の独自機能設定（誰がリアクションしたのかを非表示・リアクション数非表示・リアクション時のミュート確認）を「タイムラインとノート」から「メンタルヘルス」カテゴリに移動（#331）。

### バグ修正
- 匿名アクセスにプロフィール本文を見せない場合（`requireSigninToViewContents` 有効、またはインスタンスの UGC 公開範囲制限）でも、rel=me リンクと acct のみの最小 OGP メタデータを SSR で出力するようにし、他サーバーからの rel=me 検証バッジとリンクプレビューが機能しない問題を修正（#327）。出力するのは AP actor として既に連合へ公開されている情報のみで、bio・表示名・アバター・投稿は引き続き匿名には出さない。対象はプロフィール本体（サブページを除く）・ローカルユーザー・連合有効時に限定し、連合無効時は露出を増やさないようフォールバックする。
- やみタイムライン: やみモードがオフのとき、自分の新規やみノートがリアルタイム配信（ストリーミング）経由でやみTLに表示されてしまう問題を修正（#330）。`notes/yami-timeline` エンドポイントの「メンタルヘルス保護」（やみモードオフ時は自分の投稿も含めて非表示）とストリーミングの挙動を統一。
- 「誰がリアクションしたのかを非表示にする」設定が、通常リアクションのチップにホバーした際のユーザー一覧ツールチップに効いていなかった問題を修正（#332）。いいね（likeOnly）ツールチップ・ノート詳細のリアクションタブと同様にガードされるようになった。

## 2026.7.0-yami-1.9.39
