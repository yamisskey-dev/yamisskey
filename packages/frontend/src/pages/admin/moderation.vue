			...
			<div class="_gaps_m">
				...
				<SearchMarker :keywords="['approval', 'required', 'signup']">
					<MkSwitch v-model="approvalRequiredForSignup" @change="onChange_approvalRequiredForSignup">
						<template #label><SearchLabel>{{ i18n.ts.approvalRequiredForSignup }}</SearchLabel><span class="_beta">{{ i18n.ts.originalFeature }}</span></template>
				...
				<SearchMarker :keywords="['yami', 'note', 'federation']">
					<MkFolder>
						<template #icon><SearchIcon><i class="ti ti-exchange"></i></SearchIcon></template>
						<template #label><SearchLabel>{{ i18n.ts._settings.yamiNoteFederation }}</SearchLabel></template>
...
const emailInquiredForSignup = ref(meta.emailInquiredForSignup ?? false);
...
const yamiNoteFederationEnabled = ref(meta.yamiNoteFederationEnabled || false);
const yamiNoteFederationTrustedInstances = ref(Array.isArray(meta.yamiNoteFederationTrustedInstances)
...
function onChange_emailRequiredForSignup(value: boolean) {
	// If email is required, it must also be inquired
	if (value && !emailInquiredForSignup.value) {
		emailInquiredForSignup.value = true;
	}

	os.apiWithDialog('admin/update-meta', {
		emailInquiredForSignup: emailInquiredForSignup.value,
