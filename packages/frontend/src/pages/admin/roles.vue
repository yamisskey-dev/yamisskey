		...
		<div class="_gaps">
			<MkFolder>
				...
				<div class="_gaps_s">
					...
					<MkFolder v-if="matchQuery([i18n.ts._role._options.canUseVoiceChat, 'canUseVoiceChat'])">
						<template #label>{{ i18n.ts._role._options.canUseVoiceChat }}</template>
						<template #suffix>{{ policies.canUseVoiceChat ? i18n.ts.yes : i18n.ts.no }}</template>
						<MkSwitch v-model="policies.canUseVoiceChat">
							<template #label>{{ i18n.ts.enable }}</template>
						</MkSwitch>
					</MkFolder>
				</div>
