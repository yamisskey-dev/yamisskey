...
// 初期化
const scheduledNoteDelete = ref<DeleteScheduleEditorModelValue | null>(
	getInitialScheduledDelete(),
);
...
const parentIsYamiNote = computed(() => {
	return (
		(props.reply?.isNoteInYamiMode || props.renote?.isNoteInYamiMode) ?? false
	);
});
// デフォルト設定の変更を監視
watch(
	() => prefer.s.defaultScheduledNoteDelete,
	(newValue) => {
...
const replyTargetNote: ShallowRef<PostFormProps["reply"] | null> = shallowRef(
	props.reply,
);
...
const canPost = computed((): boolean => {
	return (
		...
		(!scheduleNote.value ||
			...
			!scheduleNote.value.scheduledAt ||
			!scheduledNoteDelete.value.deleteAt ||
...
const canSaveAsServerDraft = computed((): boolean => {
	return (
		canPost.value &&
		(textLength.value > 0 || files.value.length > 0 || poll.value != null)
	);
});
...
// 新しい状態変数を追加
const isDmIntent = ref<boolean>(
	props.initialDmIntent ??
...
const isFederationToggleDisabled = computed(() => {
	return (
		props.channel != null ||
		(visibility.value === "specified" &&
			(visibleUsers.value.length > 0 || isDmIntent.value)) || // DMの場合のみ無効化
...
if (
	...
	(replyTargetNote.value.user.username !== $i.username ||
		(replyTargetNote.value.user.host != null &&
			replyTargetNote.value.user.host !== host))
) {
...
if (
	replyTargetNote.value &&
	["home", "followers", "specified"].includes(replyTargetNote.value.visibility)
) {
	if (
		replyTargetNote.value.visibility === "home" &&
		visibility.value === "followers"
	) {
		visibility.value = "followers";
	} else if (
		["home", "followers"].includes(replyTargetNote.value.visibility) &&
		visibility.value === "specified"
	) {
		visibility.value = "specified";
	} else {
	...
	if (visibility.value === "specified") {
		...
		if (replyTargetNote.value.userId !== $i.id) {
			misskeyApi("users/show", { userId: replyTargetNote.value.userId }).then(
				(user) => {
...
function updateFileName(
	file: Misskey.entities.DriveFile,
	name: Misskey.entities.DriveFile["name"],
) {
			...
			...(replyTargetNote.value
				? {
						isReplyVisibilitySpecified:
							replyTargetNote.value.visibility === "specified",
					}
				: {}),
		},
		{
	...
	const select = await os.select({
		...
		items: [
			...
			{ value: "nonSensitiveOnly" as const, label: i18n.ts.nonSensitiveOnly },
			{
				...
				label: i18n.ts.nonSensitiveOnlyForLocalLikeOnlyForRemote,
			},
...
function saveDraft() {
	...
	const draftsData = JSON.parse(
		miLocalStorage.getItem("drafts") ?? "{}",
	) as StoredDrafts;
...
async function saveServerDraft(
	options: { isActuallyScheduled?: boolean } = {},
) {
	...
	if (scheduledNoteDelete.value) {
		if (scheduledNoteDelete.value.deleteAt) {
			...
			const baseTime =
				options.isActuallyScheduled && scheduledAt.value
					? scheduledAt.value
					: Date.now();
	...
	return await os.apiWithDialog(
		serverDraftId.value == null ? "notes/drafts/create" : "notes/drafts/update",
		{
...
async function uploadFiles() {
	...
	for (const uploadedItem of uploader.items.value.filter(
		(x) => x.uploaded != null,
	)) {
...
async function post(ev?: PointerEvent) {
	...
	if (notePostInterruptors.length > 0) {
		for (const interruptor of notePostInterruptors) {
			try {
				// やみ独自フィールド(scheduledDelete/scheduleNote/isNoteInYamiMode 等)を含むため plugin handler の期待型と不一致
				// @ts-expect-error: postData type mismatch with plugin handler expectation
...
async function openAccountMenu(ev: PointerEvent) {
	...
	function showDraftsDialog(scheduled: boolean) {
		const { dispose } = os.popup(
			defineAsyncComponent(() => import("@/components/MkNoteDraftsDialog.vue")),
			{
				scheduled,
			},
			{
				restore: async (draft: Misskey.entities.NoteDraft) => {
					...
					if (draft.poll) {
						...
						nextTick(() => {
							poll.value = {
								...
								expiresAt: draft.poll!.expiresAt
									? new Date(draft.poll!.expiresAt).getTime()
									: null,
								expiredAfter: null,
							};
						});
					...
					if (draft.visibleUserIds) {
						misskeyApi("users/show", { userIds: draft.visibleUserIds }).then(
							(users) => {
								users.forEach((u) => pushVisibleUser(u));
							},
						);
...
function showPerUploadItemMenuViaContextmenu(
	item: UploaderItem,
	ev: PointerEvent,
) {
	const menu = uploader.getMenu(item);
	...
	nextTick(() => {
		...
		if (!props.instant && !props.mention && !props.specified && !props.mock) {
			const draft = JSON.parse(miLocalStorage.getItem("drafts") ?? "{}")[
				draftKey.value
			] as StoredDrafts[string] | undefined;
			if (draft != null) {
