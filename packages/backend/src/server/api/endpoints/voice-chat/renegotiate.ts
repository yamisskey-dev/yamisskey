...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			const response = await fetch(`https://rtc.live.cloudflare.com/v1/apps/${this.config.cloudflareRealtime.appId}/sessions/${ps.sessionId}/renegotiate`, {
				method: 'PUT',
				headers: {
				...
				body: JSON.stringify({
					sessionDescription: ps.sessionDescription,
				}),
			});

			if (!response.ok) {
				throw new Error(`Failed to renegotiate session: ${response.statusText}`);
			}
